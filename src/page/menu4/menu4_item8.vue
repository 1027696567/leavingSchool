        ...
        <el-table :data="tableData">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              ...
              <el-button type="text" size="small" v-if="scope.row.memberCardAuditResStatus === 0?true:false">等待审核</el-button>
              <el-button type="text" v-if="scope.row.memberCardAuditResStatus === 1?true:false" size="small">审核通过</el-button>
              <el-button @click="findSCAuditRes(scope.row)" type="text" size="small" v-if="scope.row.memberCardAuditResStatus === -1?true:false">查看原因</el-button>
            </template>
          </el-table-column>
        </el-table>
  </div>
...
<script>
import { findMemberCardByUsername, updateMemberCardInfo } from '../../api/menu4/memberCard'
export default {
  data () {
    return {
      tableData: []
  ...
  methods: {
    findMemberCardByUsername () {
      findMemberCardByUsername({user: localStorage.getItem('user')}).then(res => {
    ...
    findSCAuditRes (row) {
      this.$alert(row.memberCardAuditResContent, '审核意见', {
        confirmButtonText: '确定'
      })
    }
  },
