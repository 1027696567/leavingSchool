...
<script>
import { updateFinalAuditRes } from '../../api/menu4/finalAuditRes'
export default {
  data () {
    return {
      dialogVisible: false,
  ...
  methods: {
    init (row) {
      this.form.stuInfoId = row.id
      this.form.id = row.finalAuditResId
    ...
    onSubmit (form) {
      this.$refs[form].validate(async (valid) => {
        if (valid) {
          ...
          await updateFinalAuditRes(this.form).then((res) => {
    ...
    parent () {
      this.$parent.findAllFinalAudits()
    },
    resetForm (form) {
      this.$refs[form].resetFields()
      this.dialogVisible = false
...
<style>
.finalAudit .el-form-item__label {
  width: 95px!important;
  padding: 0 5px 0 5px!important;
  webkit-box-sizing: border-box;
  box-sizing: border-box;
}
.finalAudit .el-form-item__content {
