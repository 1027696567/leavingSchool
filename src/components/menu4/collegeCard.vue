...
<script>
import { updateCollegeCardAuditRes } from '../../api/menu4/collegeCard'
export default {
  data () {
    return {
      dialogVisible: false,
  ...
  methods: {
    init (row) {
      this.form.id = row.collegeCardAuditResId
      this.form.stuInfoId = row.stuInfoId
      this.form.collegeCardId = row.id
    ...
    onSubmit (form) {
      this.$refs[form].validate(async (valid) => {
        if (valid) {
          ...
          await updateCollegeCardAuditRes(this.form).then((res) => {
    ...
    parent () {
      this.$parent.findAllCollegeCard()
    },
    resetForm (form) {
      this.$refs[form].resetFields()
      this.dialogVisible = false
...
<style>
.collegeCard .el-form-item__label {
  width: 95px!important;
  padding: 0 5px 0 5px!important;
  webkit-box-sizing: border-box;
  box-sizing: border-box;
}
.collegeCard .el-form-item__content {
