...
<script>
import { updateLibraryCardAuditRes } from '../../api/menu4/libraryCard'
export default {
  data () {
    return {
      dialogVisible: false,
  ...
  methods: {
    init (row) {
      this.form.id = row.libraryCardAuditResId
      this.form.stuInfoId = row.stuInfoId
      this.form.libraryCardId = row.id
    ...
    onSubmit (form) {
      this.$refs[form].validate(async (valid) => {
        if (valid) {
          ...
          await updateLibraryCardAuditRes(this.form).then((res) => {
    ...
    parent () {
      this.$parent.findAllLibraryCard()
    },
    resetForm (form) {
      this.$refs[form].resetFields()
      this.dialogVisible = false
...
<style>
.libraryCard .el-form-item__label {
  width: 95px!important;
  padding: 0 5px 0 5px!important;
  webkit-box-sizing: border-box;
  box-sizing: border-box;
}
.libraryCard .el-form-item__content {
