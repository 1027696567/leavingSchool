...
<script>
import { updateEducationAuditRes } from '../../api/menu4/education'
export default {
  data () {
    return {
      dialogVisible: false,
  ...
  methods: {
    init (row) {
      this.form.id = row.educationAuditResId
      this.form.stuInfoId = row.stuInfoId
      this.form.educationId = row.id
    ...
    onSubmit (form) {
      this.$refs[form].validate(async (valid) => {
        if (valid) {
          ...
          await updateEducationAuditRes(this.form).then((res) => {
