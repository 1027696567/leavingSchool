...
<script>
import { updateMinistryFinanceAuditRes } from '../../api/menu4/ministryFinance'
export default {
  data () {
    return {
      dialogVisible: false,
  ...
  methods: {
    init (row) {
      this.form.id = row.ministryFinanceAuditResId
      this.form.stuInfoId = row.stuInfoId
      this.form.ministryFinanceId = row.id
    ...
    onSubmit (form) {
      this.$refs[form].validate(async (valid) => {
        if (valid) {
          ...
          await updateMinistryFinanceAuditRes(this.form).then((res) => {
    ...
    parent () {
      this.$parent.findAllMinistryFinance()
    },
    resetForm (form) {
      this.$refs[form].resetFields()
      this.dialogVisible = false
...
<style>
.ministryFinance .el-form-item__label {
  width: 95px!important;
  padding: 0 5px 0 5px!important;
  webkit-box-sizing: border-box;
  box-sizing: border-box;
}
.ministryFinance .el-form-item__content {
