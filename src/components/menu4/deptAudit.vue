...
<script>
import { updateDeptAuditRes } from '../../api/menu4/api'
export default {
  data () {
    return {
      dialogVisible: false,
  ...
  methods: {
    init (row) {
      this.form.id = row.deptAuditResId
      this.form.stuInfoId = row.stuInfoId
      this.form.stuCardId = row.id
    ...
    onSubmit (form) {
      this.$refs[form].validate(async (valid) => {
        if (valid) {
          ...
          await updateDeptAuditRes(this.form).then((res) => {
...
<style>
.deptAudit .el-form-item__label {
  width: 95px!important;
  padding: 0 5px 0 5px!important;
  webkit-box-sizing: border-box;
  box-sizing: border-box;
}
.deptAudit .el-form-item__content {
